...
async function gerarPDF() {
    ...
    try {
        ...
        const alturaTopo = 15 + tamanhoTimbrado + 10;
        let y = alturaTopo;
        ...
        desenharRodape();
        doc.setFontSize(9);
        doc.text(new Date().toLocaleString(), largura - 10, 15, { align: "right" });
        doc.setFontSize(16);
        doc.text("Check List Veicular Diário", largura / 2, 15 + tamanhoTimbrado / 2, { align: "center", baseline: "middle" });
        ...
        function desenharTabela(perguntasGrupo, colunas) {
            const larguraCol = (largura - 20 - (colunas - 1) * 5) / colunas;
            for (let i = 0; i < perguntasGrupo.length; i += colunas) {
                if (y + 10 > altura - rodapeAltura * 2 - 20) novaPagina();
                let x = 10;
                let alturaLinha = 8;
                for (let j = 0; j < colunas; j++) {
                    const p = perguntasGrupo[i + j];
                    if (p) {
                        const texto = doc.splitTextToSize(`${p.num}. ${p.label} ${p.value || ''}`, larguraCol - 2);
                        alturaLinha = Math.max(alturaLinha, texto.length * 4 + 2);
                        doc.rect(x - 1, y - 1, larguraCol + 2, alturaLinha);
                        doc.text(texto, x + larguraCol / 2, y + 1, { align: "center" });
                        x += larguraCol + 5;
                    }
                }
                y += alturaLinha + 2;
        ...
        desenharTabela(perguntas.slice(0, 3), 3);
        desenharTabela(perguntas.slice(3, 36), 3);

        y += 10;
        if (y + 55 > altura - rodapeAltura * 2 - 20) novaPagina();
        ...
        if (y + 50 > altura - rodapeAltura * 2 - 20) novaPagina();
        ...
        for (let file of arquivos) {
            ...
            if (y + tamanhoImagem > altura - rodapeAltura * 2 - 20) novaPagina();
        ...
        if (y + 40 > altura - rodapeAltura * 2 - 20) novaPagina();
